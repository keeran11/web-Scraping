...
total_pages = 4  # 4 pages to scrape
...
def parsing_results(page):
    ...
    for product in products:
        ...
        try:
            sold_element = product.query_selector('div[class*=rating-wrapper] > div:nth-child(3)')  # Update class based on Daraz's HTML structure for sold
            print(f"sold_element: {sold_element.inner_html()}")  # Debug print
...
with sync_playwright() as p:
    ...
    try:
        for page_number in range(1, total_pages + 1):
            ...
            try:
                page.wait_for_load_state('domcontentloaded')  # network idle
            except Exception as e:
